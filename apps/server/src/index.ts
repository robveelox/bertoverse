...
import { createSession, destroySession, parseCookies, registerUser, SESSION_COOKIE, userById, userFromRequest, userFromToken, verifyLogin } from './auth.js';
...
app.get('/api/rooms', async (req, res) => {
  ...
  const [publicRooms] = await pool.query<any[]>(`SELECT r.public_id AS publicId, r.name, r.description, r.layout_key AS layoutKey,
    r.max_visitors AS maxVisitors, r.created_at AS createdAt, u.username AS ownerName
  ...
  if (user) {
    [mine] = await pool.query<any[]>(`SELECT public_id AS publicId, name, description, layout_key AS layoutKey, max_visitors AS maxVisitors, created_at AS createdAt
      FROM rooms WHERE owner_user_id = ? ORDER BY created_at DESC LIMIT 30`, [user.id]);
    [recent] = await pool.query<any[]>(`SELECT r.public_id AS publicId, r.name, r.description, r.layout_key AS layoutKey, r.max_visitors AS maxVisitors, r.created_at AS createdAt
  ...
  const count = (rooms: any[]) => rooms.map(room => ({ ...room, occupancy: occupancy.get(room.publicId) ?? 0 }));
  res.json({ public: count(publicRooms), mine: count(mine), recent: count(recent) });
});
...
app.post('/api/rooms', async (req, res) => {
  ...
  res.status(201).json({ room: { publicId, name, description, layoutKey, maxVisitors: 25, occupancy: 0 } });
});
...
app.post('/api/catalog/:id/purchase', async (req, res) => {
  const user = await userFromRequest(req);
...
io.on('connection', (socket) => {
  socket.on('join', async (payload: { name?: unknown; roomId?: unknown } = {}) => {
    ...
    const [roomRows] = await pool.query<any[]>(`SELECT r.public_id AS publicId, r.name, r.width, r.height, r.created_at AS createdAt,
  ...
  socket.on('room:join', async (roomId: unknown) => {
    ...
    if (!player) return;
    const [rows] = await pool.query<any[]>(`SELECT r.id, r.public_id AS publicId, r.name, r.width, r.height, r.created_at AS createdAt,
      u.username AS ownerName FROM rooms r LEFT JOIN users u ON u.id = r.owner_user_id
  ...
  socket.on('move', (target: Tile) => {
    ...
    io.to(player.roomId).emit('player:path', { playerId: player.id, path });
    let step = 0;
    const advance = () => {
      ...
      if (step >= path.length) {
        ...
        const timer = setTimeout(advance, diagonal ? 212 : 150);
        movers.set(socket.id, timer);
      }
    };
    const first = path[0];
    const timer = setTimeout(advance, first && player.x !== first.x && player.y !== first.y ? 212 : 150);
    movers.set(socket.id, timer);
  });
  ...
  socket.on('chat', (raw: unknown) => {
    const player = players.get(socket.id);
    const now = Date.now();
    if (!player || now - (lastChat.get(socket.id) ?? 0) < 750) return;
